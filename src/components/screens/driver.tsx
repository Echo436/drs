...
import { DriverStanding, Race } from '@/src/context/F1DataContext'
...
import { useConstructorStandingsQuery } from '@/src/hooks/useF1Queries'
...
export default function DriverDetail() {
  ...
  const navigateToTeam = (constructorId: string) => {
    const constructor = constructorList.find(
      (c: { Constructor: { constructorId: string } }) =>
        c.Constructor.constructorId === constructorId,
    )
    router.navigate({
      pathname: './teamModal',
      params: {
        ...
        <View style={styles.profileContainer}>
          ...
          <View style={styles.rightColumn}>
            ...
            <Host matchContents style={{ width: 200, marginTop: 6 }}>
              <Button
                modifiers={[buttonStyle('glassProminent'), tint(teamColor)]}
                onPress={() =>
                  navigateToTeam(
              ...
              >
                <Text
                  modifiers={[
                    foregroundStyle(
                      tinycolor(teamColor).isLight() ? '#000000' : '#FFFFFF',
                    ),
                    font({ weight: 'medium' }),
                  ]}
                >
