...
import { Result } from '@/src/context/F1DataContext'
import { Link, router, Stack, useLocalSearchParams } from 'expo-router'
import { ScrollView } from 'react-native-gesture-handler'
import { layoutStyles, cardStyles } from '@/src/components/ui/Styles'
...
import Svgtest from '@/assets/images/circuit-simple/2026/albert_park.svg'
import { getTeamsColor } from '@/src/constants/Colors'
import { getCircuitImage } from '@/src/constants/CircuitImages'
import {
...
export default function GrandPrixDetail() {
  ...
  const { data: raceData } = useRaceDetailsQuery(
    initRaceData.season,
    initRaceData.round,
    JSON.parse(initialData),
  )
  const { data: extraRaceData, isSuccess: extraRaceDataSuccess } =
    useExtraRaceDetailsQuery(initRaceData.season, initRaceData.round)
  const { data: qualyResultData } = useQualiResultsQuery(
    initRaceData.season,
    initRaceData.round,
  )
  const { data: sprintResultData } = useSprintResultsQuery(
    initRaceData.season,
    initRaceData.round,
  )
  const { data: raceResultData } = useRaceResultsQuery(
    initRaceData.season,
    initRaceData.round,
  )
  ...
  const onRefresh = async () => {
    ...
    try {
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: f1apiKeys.raceDetails(
            initRaceData.season,
            initRaceData.round,
          ),
        }),
  ...
  const circuitImageAsset = getCircuitImage(
          ...
          <View>
            ...
            >
              ...
              >
                ...
                >
                  {circuitImageAsset?.type === 'svg' ? (
                    <circuitImageAsset.Svg
                      width="100%"
                      height="100%"
                      fill={svgColor}
                    />
                  ) : circuitImageAsset?.type === 'bitmap' ? (
                    <Image
                      resizeMode="contain"
                      style={{
                        width: '100%',
                      ...
                      source={circuitImageAsset.source}
                    />
                  ) : null}
                </View>
                <IconSymbol
                            ...
                            <TouchableOpacity
                              ...
                              disabled={
                                !(
                                  (item.key === 'sprintRace' &&
                                    sprintResultData) ||
                                  (item.key === 'qualy' && qualyResultData) ||
                                  (item.key === 'race' && raceResultData)
                                )
                              }
                            >
                              <View style={{ flex: 1 }}>
