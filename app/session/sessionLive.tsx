...
const fetchPositionData = async (
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/position?session_key=${sessionKey}`,
    )
...
const fetchCarData = async (sessionKey: number = 10006): Promise<CarData[]> => {
  try {
    const res = await fetch(
      `${API_BASE_URL}/car_data?session_key=${sessionKey}`,
    )
    return await res.json()
  } catch (err) {
...
const fetchIntervalData = async (
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/intervals?session_key=${sessionKey}`,
    )
    return await res.json()
  } catch (err) {
...
const RaceMonitor = ({
  ...
  const [intervals, setIntervals] = useState<
    Record<number, Interval | undefined>
  >({})
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      // 并行获取所有数据
      const [positionData, carDataResult, intervalData, lapData, pitData] =
        await Promise.all([
      ...
      if (positionData.length > 0) {
        drivers.forEach((driver) => {
          const latest = [...positionData]
            .reverse()
            .find((pos) => pos.driver_number === driver.driver_number)
      ...
      if (carDataResult.length > 0) {
        drivers.forEach((driver) => {
          const latest = [...carDataResult]
            .reverse()
            .find((data) => data.driver_number === driver.driver_number)
          ...
          <View style={styles.itemContainer}>
            <View style={styles.positionContainer}>
              <Text style={styles.positionText}>
                {positions[item.driver_number]?.position ?? '-'}
              </Text>
